...
document.write('<body>' +
    '<div class="container" id="content"><h1>Welcome to the FIDO20-Democlient!</h1>\n' +
    ...
    '    <p>Please note that this client is pretty simple. Check the comments in <strong>script/spa-demo.js</strong> to learn more about it. However, hopefully it does help to understand how a FIDO2 flow in a SPA works!</p>' +
    '    <p>This demo is also used to show how the <a href="https://konghq.com" target="_blank"><strong>Kong</strong></a> validation plugin can be used.</p>' +
    '    <p>This demo makes a request to Kong which returns an error if no FIDO2 based credential is included in the request.</p>' +
...
function getLoginMsgResponse() {
    ...
    // return '<p>Please login before accessing exciting content!</p>\n' +
    //     '<strong>Sign-In: </strong><input type="email" id="idSignInName" name="signInName" size="35" placeholder="Your LoginID Username/EMail">\n' +
    //     '<button type="button" value="check" id="idBtnSignInName" name="btnSignInName" onclick="return signInUser();">' +
    //     '<img src="../images/button_loginid.png" alt="sign-in" width="70px"/></button>';
}

function getMessageFooter() {
    return '<hr/><p>Try again! <a href="democlientApp.jsp"><strong>Web Application!</strong></a>, <a href="index.html"><strong>SPA!</strong></a></p>'
}
...
function printFirstPage() {
    let content = '<h3>APIs</h3>' +
        '<p><strong>https://catfact.ninja</strong>: the public API which is called directly.</p>' +
        '<p><strong>http://localhost:8090/facts</strong>: the same public API but hosted on Kong as a protected API. A request to this API requires a valid FIDO2 credential in the form of a JSON Web Token (JWT), issued by <strong>LoginID</strong>.</p>' +
        '<table>' +
        '<tr><td><label for="btnDirect">Direct request:</label></td><td><button id="btnDirect" type="button" class="btn btn-primary btn-sm" onclick="return directCall(\'https://catfact.ninja/fact\');">https://catfact.ninja</button></td><td>(client --> cat-fact-API)</td></tr>' +
        '<tr><td><label for="btnProtected">Protected request:</label></td><td><button id="btnProtected" type="button" class="btn btn-primary btn-sm" onclick="return protectedCall(\'http://localhost:8090/fact\');">http://localhost:8090/fact</button></td><td>(client --> Kong --> cat-fact-API)</td></tr>' +
        '</table>' +
        '<h3>API Responses</h3>' +
